...
import { PostsDataContainer } from "./phoneStyles";
    ...
    const fetchData = makeApiCall(`/getTopics/${user?.role}`)
      .then((responseData) => {
        if (responseData.message !== "no post available yet") {
          const phoneTopics = responseData.message.filter(
            (topic) => topic?.category === "phone and technology"
          );
          setGetTopics(phoneTopics);
          setLoading(false);
        } else {
          setGetTopics([]);
          setLoading(false);
